...
import com.example.appmarte.databinding.ItemFarmaciasBinding
...
class AdapterMars  : RecyclerView.Adapter<AdapterMars.MarsVH>() {
    ...
    //1 INNER CLASS
    inner class MarsVH(private val binding: ItemFarmaciasBinding) :
        ...
        fun bin(mars: MarsRealState) {
           ...
           // Glide.with(binding.imageView2).load(mars.img_Src).centerCrop().into(binding.imageView2)
            // Activar el Clik
          binding.txtfamacia.text = " FARMACIA " + mars.localnombre.toString()
           binding.txtdireccion.text =  "  "+ mars.localdireccion.toString()
           binding.txtcomuna.text = "  "+mars.comunanombre.toString()

//            binding.txtfamacia.text = mars.id.toString()
//            binding.txtdireccion.text =  mars.name.toString()
//            binding.txtcomuna.text = mars.price.toString()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MarsVH {
        return MarsVH(ItemFarmaciasBinding.inflate(LayoutInflater.from(parent.context)))
    }
